...
function MyVerticallyCenteredModal(props) {
  ...
  `;
        ...
        let result = await fetch("http://localhost:5000/api/addfilms", {
            ...
            body: JSON.stringify({ username: localStorage.getItem("Username"), movieid: props.id.id, review: review, rating: star, date: date, type: "movie" })
        }
...
function Card({ log, setlog }) {
    ...
    useEffect(() => {
        fetch("https://api.themoviedb.org/3/movie/" + id.id + "?api_key=7df67dc3f76c8534357b9da39a1d3afc&language=en-US")
            .then((res) => res.json())
            .then((dat) => {
                setmovie(dat)
                console.log(dat);
