...
import { Link, BrowserRouter as Router, Route, Routes, useAsyncError, } from 'react-router-dom';
import Home from './Home';
import { useState, useEffect } from 'react';
import axios from 'axios';
import Dropdown from 'react-bootstrap/Dropdown';
import CardSkeleton from './CardSkeleton.js';
function Movie({ log, setlog }) {
    ...
    useEffect(() => {
        try {
            fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&page=${pageno}`)
                .then(response => {
                    if (response.ok) {
    ...
    const searchPage = async () => {
        try {
            const response = await axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&page=${pageno}`);
            setSearchResults(response.data.results);
        } catch (error) {
            console.error('Error searching movies:', error);
        }
        ...
        <div className='main'>
            <div className='main1'>
                ...
                {isLoading ? (
                        ...
                        // Render movie cards when isLoading is false
                        searchResults.map((movie) => (
                            <Link style={{ textDecoration: "none" }} to={'/card/'+movie.id}>
                                <div key={movie.id} >
                                    <div className="card" style={{ width: '300px', margin: '10px', padding: '10px', border: '1px solid #ccc', borderRadius: '5px', height: '600px' }} >
                                        <img className="card-img-top" src={`https://image.tmdb.org/t/p/w400/${movie.poster_path}`} style={{minWidth:"278px", minHeight:"400px"}} alt={`${movie.title} Poster`} />
                                        <div className="card-body">
                                            <h5 className="card-title">{movie.title}</h5>
                                            <p className="card-text">{truncateOverview(movie.overview, 20)}</p>
                                        </div>
                                    </div>
                                </div>
                            </Link>
